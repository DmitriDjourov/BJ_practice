...
//39. В Указанном массиве вещественных чисел найдите разницу между максимальным и минимальным элементом
        ...
        //      Создаем массив
        //      Заполняем массив
        // 2. Находим разницу между максимальным и минимальным элементом
        // 3. Выводим на печать итог
...
//        Заполняем массив ручками
        fillArray(array, sc);
//      2. Находим разницу между максимальным и минимальным элементом
        double difference = findDiffBetweenMaxAndMin(array);
//      3. Выводим на печать итог
        System.out.printf("Разница между максимальным и минимальным элементом: %.1f" , difference);
    }

    // Метод для создания массива размером N
    static public double[] createArray(int N) {
        return new double[N];
    }

    // Метод для заполнения массива пользователем
    static public void fillArray(double[] array, Scanner sc) {
    ...
    //  Метод для определения разницы между максимальным и минимальным элементом
    static public double findDiffBetweenMaxAndMin(double[] array) {
        // Определяем что массив заполнен
        if (array == null || array.length == 0) {
            throw new IllegalArgumentException("Массив не должен быть пустым.");
        }
        // задаем значения для минимальной и максимальной величины
        double max = array[0];
        double min = array[0];
        // определяем минимальные и максимальные значенияL
        for (int i = 0; i < array.length; i++) {
            if (array[i] < min) {
                min = array[i];
            }
